if type(_G.LunarHubCleanup) == "function" then
    pcall(_G.LunarHubCleanup)
    _G.LunarHubCleanup = nil
end

for _, name in pairs({"LunarHub", "LunarHubWarning"}) do
...
if _G.LH then
    if _G.LH.lunar and _G.LH.lunar.running then
        _G.LH.lunar.running = false
        if _G.LH.lunar.thread then pcall(task.cancel, _G.LH.lunar.thread) end
        _G.LH.lunar.thread = nil
    end
    _G.LH = nil
end

if workspace:FindFirstChild("LunarHubTpCircle") then
    workspace.LunarHubTpCircle:Destroy()
end
...
if game.PlaceId ~= 13822889 then
    task.spawn(function()
        ...
        icon.TextSize = 42; icon.TextColor3 = Color3.fromRGB(110, 170, 255); icon.Text = "!"
...
local THEME_TEXT   = Color3.fromRGB(120, 180, 255)
local BTN_COLOR    = Color3.fromRGB(8, 12, 36)
local BTN_HOVER    = Color3.fromRGB(16, 26, 72)
local ACCENT       = Color3.fromRGB(95, 150, 230)
local BG_DARK      = Color3.fromRGB(4, 6, 18)
local BG_SIDE      = Color3.fromRGB(6, 9, 28)
local BG_TOP       = Color3.fromRGB(5, 7, 20)
local BORDER_COLOR = Color3.fromRGB(28, 50, 120)
local SEP_COLOR    = Color3.fromRGB(20, 35, 90)
local SECTION_TEXT = Color3.fromRGB(80, 130, 210)
local OUTER_BG     = Color3.fromRGB(8,   8,   8 )

local SW_OFF      = Color3.fromRGB(25, 45, 110)
local SW_ON       = Color3.fromRGB(130, 185, 255)
local SW_KNOB_OFF = Color3.fromRGB(95, 150, 230)
local SW_KNOB_ON  = Color3.fromRGB(15, 22, 65)

local PB_BAR  = Color3.fromRGB(180, 220, 255)
local PB_TEXT = Color3.fromRGB(180, 220, 255)
...
local lunarThread  = nil

local function onExit()
    lunarRunning = false
    if lunarThread then pcall(task.cancel, lunarThread); lunarThread = nil end
    if _G.LH and _G.LH.lunar then
        _G.LH.lunar.running = false
        if _G.LH.lunar.thread then pcall(task.cancel, _G.LH.lunar.thread); _G.LH.lunar.thread = nil end
    end
...
local _mainBgImage = Instance.new("ImageLabel", main)
_mainBgImage.Size = UDim2.new(1, 0, 1, 0)
_mainBgImage.Position = UDim2.new(0, 0, 0, 0)
_mainBgImage.BackgroundTransparency = 1
_mainBgImage.Image = "rbxassetid://138962018382791"
_mainBgImage.ScaleType = Enum.ScaleType.Crop
_mainBgImage.ImageTransparency = 0.6
_mainBgImage.ZIndex = -1
...
topBarSep.ZIndex = 5
local _topBgImage = Instance.new("ImageLabel", topBar)
_topBgImage.Size = UDim2.new(1, 0, 1, 0)
_topBgImage.Position = UDim2.new(0, 0, 0, 0)
_topBgImage.BackgroundTransparency = 1
_topBgImage.Image = "rbxassetid://94577501976799"
_topBgImage.ScaleType = Enum.ScaleType.Crop
_topBgImage.ImageTransparency = 0.3
_topBgImage.ZIndex = 0
...
hubIcon.Image = "rbxassetid://89575909218441"
...
titleLbl.BackgroundTransparency = 1; titleLbl.Text = "LunarHub | LT2"
...
versionLbl.TextColor3 = Color3.fromRGB(80, 130, 210); versionLbl.TextXAlignment = Enum.TextXAlignment.Right
versionLbl.ZIndex = 5
...
if isMobile then
    mobileToggleBtn = Instance.new("ImageButton", gui)
    mobileToggleBtn.Name = "LHMobileToggle"
    mobileToggleBtn.Size = UDim2.new(0, 52, 0, 52)
    mobileToggleBtn.Position = UDim2.new(0, 12, 0.5, -26)
    mobileToggleBtn.BackgroundColor3 = Color3.fromRGB(10, 14, 42)
    mobileToggleBtn.BorderSizePixel = 0
    mobileToggleBtn.Image = "rbxassetid://89575909218441"
    mobileToggleBtn.ScaleType = Enum.ScaleType.Fit
    mobileToggleBtn.ImageColor3 = Color3.fromRGB(120, 180, 255)
    ...
    local mtStroke = Instance.new("UIStroke", mobileToggleBtn)
    mtStroke.Color = BORDER_COLOR; mtStroke.Thickness = 1.5; mtStroke.Transparency = 0.3

    local mtDragging, mtDragStart, mtStartPos = false, nil, nil
    mobileToggleBtn.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch then
            mtDragging = true; mtDragStart = input.Position; mtStartPos = mobileToggleBtn.Position
        end
    end)
    UserInputService.InputChanged:Connect(function(input)
        if mtDragging and input.UserInputType == Enum.UserInputType.Touch then
            local delta = input.Position - mtDragStart
            mobileToggleBtn.Position = UDim2.new(mtStartPos.X.Scale, mtStartPos.X.Offset + delta.X, mtStartPos.Y.Scale, mtStartPos.Y.Offset + delta.Y)
        end
    end)
...
minimizeBtn.BackgroundColor3 = Color3.fromRGB(10, 14, 42)
minimizeBtn.BorderSizePixel = 0
minimizeBtn.Font = Enum.Font.GothamBold
minimizeBtn.TextSize = 15
minimizeBtn.TextColor3 = Color3.fromRGB(75, 120, 200)
minimizeBtn.Text = "−"
...
minimizeBtn.MouseEnter:Connect(function()
    TweenService:Create(minimizeBtn, TweenInfo.new(0.15), {
        BackgroundColor3 = Color3.fromRGB(15, 22, 65),
        TextColor3 = Color3.fromRGB(100, 160, 240)
    }):Play()
end)
minimizeBtn.MouseLeave:Connect(function()
    TweenService:Create(minimizeBtn, TweenInfo.new(0.15), {
        BackgroundColor3 = Color3.fromRGB(10, 14, 42),
        TextColor3 = Color3.fromRGB(75, 120, 200)
    }):Play()
end)
minimizeBtn.MouseButton1Click:Connect(function()
    if isMobile then
        ...
        TweenService:Create(wrapper, TweenInfo.new(0.28, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
...
side.ScrollBarImageColor3 = Color3.fromRGB(40, 70, 150)
...
task.spawn(function()
    ...
    wf.BackgroundColor3 = Color3.fromRGB(11, 16, 48); wf.BackgroundTransparency = 1; wf.BorderSizePixel = 0
...
for _, name in ipairs(tabs) do
    ...
    page.ScrollBarImageColor3 = Color3.fromRGB(40, 70, 150)
    page.Visible = false
...
local function switchTab(targetName)
    ...
    if activeTabButton then
        ...
        if oldLbl  then TweenService:Create(oldLbl,  TweenInfo.new(0.22), {TextColor3  = Color3.fromRGB(70, 110, 190)}):Play() end
        if oldIcon then TweenService:Create(oldIcon, TweenInfo.new(0.22), {ImageColor3 = Color3.fromRGB(70, 110, 190)}):Play() end
    end
    ...
    if frame then
        ...
        TweenService:Create(frame, TweenInfo.new(0.22), {BackgroundColor3 = Color3.fromRGB(18, 30, 82)}):Play()
...
for _, name in ipairs(tabs) do
    ...
    icon.ImageColor3 = Color3.fromRGB(70, 110, 190)
    icon.ZIndex = 3
    ...
    nameLbl.TextSize = 13
    nameLbl.TextColor3 = Color3.fromRGB(70, 110, 190)
    ...
    btn.MouseEnter:Connect(function()
        if activeTabButton ~= frame then
            TweenService:Create(frame,   TweenInfo.new(0.18), {BackgroundColor3 = Color3.fromRGB(12, 18, 52)}):Play()
            TweenService:Create(nameLbl, TweenInfo.new(0.18), {TextColor3       = Color3.fromRGB(100, 160, 240)}):Play()
            TweenService:Create(icon,    TweenInfo.new(0.18), {ImageColor3      = Color3.fromRGB(100, 160, 240)}):Play()
        end
    end)
    btn.MouseLeave:Connect(function()
        if activeTabButton ~= frame then
            TweenService:Create(frame,   TweenInfo.new(0.18), {BackgroundColor3 = Color3.fromRGB(0, 0, 0)}):Play()
            TweenService:Create(nameLbl, TweenInfo.new(0.18), {TextColor3       = Color3.fromRGB(70, 110, 190)}):Play()
            TweenService:Create(icon,    TweenInfo.new(0.18), {ImageColor3      = Color3.fromRGB(70, 110, 190)}):Play()
        end
    end)
...
task.spawn(function()
    ...
    overlay.Name              = "LHLoadingOverlay"
    overlay.Size              = UDim2.new(1, 0, 1, -40)
    overlay.Position          = UDim2.new(0, 0, 0, 40)
    overlay.BackgroundColor3  = Color3.fromRGB(4, 6, 18)
    ...
    titleLbl2.TextColor3        = Color3.fromRGB(120, 180, 255)
    titleLbl2.TextXAlignment    = Enum.TextXAlignment.Center
    titleLbl2.Text              = "Loading LunarHub"
    titleLbl2.ZIndex            = 51
    ...
    subLbl.TextSize          = 12
    subLbl.TextColor3        = Color3.fromRGB(60, 100, 180)
    ...
    barTrack.BackgroundColor3 = Color3.fromRGB(15, 22, 65)
    ...
    pctLbl.TextSize          = 11
    pctLbl.TextColor3        = Color3.fromRGB(80, 130, 210)
...
local function toggleGUI()
    ...
    if guiOpen then
        if mobileToggleBtn then mobileToggleBtn.Visible = false end
        main.Visible = true
        main.Size = UDim2.new(0, 0, 0, 0)
        main.BackgroundTransparency = 1
        wrapper.Size = UDim2.new(0, 0, 0, 0)
        ...
        t.Completed:Connect(function()
            ...
            if isMobile and mobileToggleBtn then mobileToggleBtn.Visible = true end
        end)
    end
end
...
bubbleIcon.ScaleType=Enum.ScaleType.Fit; bubbleIcon.Image="rbxassetid://89575909218441"
...
iconName.TextColor3=THEME_TEXT; iconName.TextXAlignment=Enum.TextXAlignment.Center; iconName.Text="Lunar"
...
local function createStatusBox(text, color)
    ...
    box.BackgroundColor3 = Color3.fromRGB(11, 16, 48); box.BorderSizePixel = 0
...
local lagLabel   = createStatusBox("Lag: ...", Color3.fromRGB(100, 160, 240))
createStatusBox("Acc Age: " .. player.AccountAge .. "d")
local execLabel  = createStatusBox("Exec: detecting...", Color3.fromRGB(110, 170, 255))
...
lagThread = task.spawn(function()
    while gui and gui.Parent do
        ...
        if lagLabel and lagLabel.Parent then
            if ok then
                if ping > 250 then
                    lagLabel.Text = "Bad Ping"; lagLabel.TextColor3 = Color3.fromRGB(110, 170, 255)
                elseif ping > 120 then
                    lagLabel.Text = "High Ping"; lagLabel.TextColor3 = Color3.fromRGB(100, 160, 240)
                else
                    lagLabel.Text = "Good Ping"; lagLabel.TextColor3 = Color3.fromRGB(120, 180, 255)
                end
            else
...
local siUptimeLabel  = createServerInfoBox("Uptime: 00:00:00", Color3.fromRGB(160, 200, 255))
local siPlayersLabel = createServerInfoBox("Players: ...", Color3.fromRGB(110, 170, 255))
...
discordFrame.BackgroundColor3 = Color3.fromRGB(11, 16, 48)
...
discordLabel.TextColor3 = Color3.fromRGB(95, 150, 230); discordLabel.TextXAlignment = Enum.TextXAlignment.Left
...
copyStroke.Color = Color3.fromRGB(25, 45, 110); copyStroke.Thickness = 1; copyStroke.Transparency = 0
...
_G.LH = {
    ...
    switchTab         = switchTab,
    toggleGUI         = toggleGUI,
    lunar = { running = false, thread = nil },
    isMobile          = isMobile,
    isFlyActive       = false,
    flyEnabled        = true,
